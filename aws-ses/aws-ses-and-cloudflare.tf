...
# Amazon changed verification from TXT to CNAME.
resource "cloudflare_record" "cloudflare_record_domain" {
  ...
  name  = "${element(aws_ses_domain_dkim.domain_dkim.dkim_tokens, count.index)}._domainkey.${aws_ses_domain_identity.identity_domain.id}"
